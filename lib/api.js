import { GYM_WEBSITE_API_URL, TERMS_API_URL, WEBSITE_GALLERY_ITEMS_API_URL, WEBSITE_PRICING_ITEMS_API_URL } from "./constants";
...
export async function getGalleryByGym(gymId) {
  if (!gymId) return [];
  const res = await fetch(
    `${WEBSITE_GALLERY_ITEMS_API_URL}/public?gym=${gymId}&active=true`,
    { next: { revalidate: 60 } },
  );
  if (!res.ok) return [];
  return res.json();
...
export async function getTermsByGym(gymId) {
  if (!gymId) return null;
  const res = await fetch(`${TERMS_API_URL}/public?gym=${gymId}`, {
    next: { revalidate: 300 },
  });
  if (!res.ok) return null;
